...
void main () {
	...
	OFB_Enc_Dec(IV, m, bl, result, c);
	
	
	
	FILE* outputFile = fopen("ofb_cipher.txt", "w");
		if (outputFile == NULL) {
	...
	for (int i=0;i<16;i++) fprintf(outputFile, "%02x ", IV[i]);
	
	fprintf(outputFile, "\n\nThe Encrypted Message is (using OFB mode with AES-128 Encryption) :-");
