...
void main () {
	...
	FILE *cipherFile = fopen("cbc_cipher.txt", "r");
    	if (cipherFile == NULL) {
	...
	for (int i=0;i<16;i++) fscanf(cipherFile, "%02hhx ", &IV[i]); // Scanning the IV first.
	...
	FILE *plaintextFile = fopen("cbc_decrypt.txt", "w");
